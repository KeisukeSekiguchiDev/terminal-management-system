...
### 1.2 命名規則
- テーブル名: スネークケース、複数形（例: terminals）
- カラム名: スネークケース（例: serial_number）
- インデックス名: idx_テーブル名_カラム名（例: idx_terminals_serial_number）
- 外部キー名: fk_テーブル名_参照テーブル名（例: fk_terminals_customers）
...
-- コメント
COMMENT ON TABLE customers IS '顧客企業マスター';
COMMENT ON COLUMN customers.contract_type IS 'basic:ベーシック, standard:スタンダード, premium:プレミアム';
```
...
CREATE TABLE terminals (
    ...
    -- 設定
    heartbeat_interval INTEGER DEFAULT 300, -- 秒
...
-- コメント
COMMENT ON TABLE terminals IS '端末マスター';
COMMENT ON COLUMN terminals.status IS 'online:オンライン, offline:オフライン, error:エラー, maintenance:メンテナンス';
```

### 2.3 terminal_logs（端末ログ）
...
-- コメント
COMMENT ON COLUMN alerts.severity IS 'CRITICAL:緊急, HIGH:高, MEDIUM:中, LOW:低, INFO:情報';
COMMENT ON COLUMN alerts.alert_type IS 'offline,error,high_cpu,high_memory,update_failed,etc';
```

### 2.5 firmware_versions（ファームウェアバージョン）
...
-- コメント
COMMENT ON COLUMN update_tasks.task_type IS 'firmware:ファームウェア更新, config:設定更新, reboot:再起動, etc';
COMMENT ON COLUMN update_tasks.status IS 'pending:待機中, running:実行中, completed:完了, failed:失敗, cancelled:キャンセル';
```
...
-- コメント
COMMENT ON COLUMN users.role IS 'admin:管理者, operator:オペレーター, support:サポート, viewer:閲覧者';
```
...
### 3.1 v_terminal_summary（端末サマリービュー）
...
### 3.2 v_daily_statistics（日次統計ビュー）
...
('admin', 'admin@techcore.com', 'システム管理者', 'admin', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/Lewvfmn0ma/SlWpC.'), -- password: admin123
('operator1', 'operator1@techcore.com', '運用オペレーター1', 'operator', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/Lewvfmn0ma/SlWpC.'),
('support1', 'support1@techcore.com', 'サポート担当1', 'support', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/Lewvfmn0ma/SlWpC.');
```
...
('株式会社ABCストア', '山田太郎', 'yamada@abc-store.jp', '03-1234-5678', 'premium', '2025-01-01'),
('XYZ商事', '鈴木花子', 'suzuki@xyz.co.jp', '06-9876-5432', 'standard', '2025-01-01'),
('田中商店', '田中一郎', 'tanaka@tanaka-shop.jp', '052-555-1234', 'basic', '2025-01-01');

-- テスト用端末（実際のデータ投入はエージェントから）
...
DATABASES = {
    'default': {
        ...
        'NAME': 'tms_db',
        'USER': 'tms_user',
        'PASSWORD': 'tms_password_dev_2025',  # 本番では環境変数から取得
        'HOST': 'localhost',
        'PORT': '5432',
        'OPTIONS': {
            'connect_timeout': 10,
...
この設計書に基づいて、Devinが自動的にデータベースを構築できます。